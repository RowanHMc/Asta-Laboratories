        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="datetime-local" required value={form.bookingTime} onChange={updateForm("bookingTime")} className="w-full p-2.5 text-xs bg-[#f8faf9] border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#064e3b]" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">Experiment to be Done</label>
            <input type="text" required placeholder="e.g. DNA Extraction and PCR Gel Electrophoresis" value={form.experiment} onChange={updateForm("experiment")} className="w-full p-2.5 text-xs bg-[#f8faf9] border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#064e3b]" />
          </div>
...
{/* table  */}
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">{isAdmin ? "All Student Lab Requests" : "My Submitted Requests"}</h3>
          ...
          <thead className="bg-slate-50 text-slate-500 uppercase border-b border-slate-200">
            ...
            {rows.map((req) => (
              <tr key={req.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-3">
                  <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-full border uppercase ${STATUS_STYLE[req.status]}`}>{req.status}</span>
                </td>
                {isAdmin && (
                  <td className="p-3 text-right space-x-2">
                    <button onClick={() => setStatus(req.id, "approved")} disabled={req.status === "approved"} className="p-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg border border-emerald-200 disabled:opacity-30 cursor-pointer" title="Approve Booking">
                      <Check className="w-4 h-4" />
                    </button>
                    <button onClick={() => setStatus(req.id, "rejected")} disabled={req.status === "rejected"} className="p-1.5 bg-red-50 text-red-700 hover:bg-red-100 rounded-lg border border-red-200 disabled:opacity-30 cursor-pointer" title="Reject Booking">
                      <X className="w-4 h-4" />
                    </button>
                  </td>
                )}
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={isAdmin ? 6 : 4} className="p-4 text-center text-slate-400">No reservation requests found.</td>
              </tr>
            )}
          </tbody>
        </table>
