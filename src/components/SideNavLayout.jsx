            ...
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {!isCollapsed && (
                <div className="truncate">
                  <span className="font-bold text-slate-900 text-sm tracking-tight block">ASTA LABS</span>
                  <span className="text-[10px] text-slate-400 font-medium block">Research Portal</span>
                </div>
              )}
            </div>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-medium transition-all ${
                    isActive ? "bg-[#064e3b] text-white shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                  }`}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="hidden sm:block text-left">
                ...
                <p className="text-[10px] text-slate-400 font-medium leading-none">
                  {userRole === "admin" ? "Admin Account" : "Student Account"}
                </p>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  {notifications && notifications.length > 0 ? (
                    notifications.map((item) => (
                      <div key={item.id} className="p-3 bg-slate-50 border border-slate-100 rounded-xl text-xs">
                        <p className="font-semibold text-slate-800">{item.title}</p>
                        <p className="text-slate-500 text-[11px] mt-0.5">{item.message}</p>
                        <span className="text-[10px] text-slate-400 mt-2 block">{item.timestamp}</span>
                      </div>
                    ))
                  ) : (
                    <div className="py-12 text-center text-slate-400">
                      ...
                      <p className="text-[10px] text-slate-400 mt-1">Admin updates and alerts will appear here.</p>
                    </div>
                  )}
                </div>
              </div>
